...
it('it should edit activity ', function(done) {
    if(activityDetail['id']!='' && activityDetail['id']!=null) {
        activityDetail['state'] = "APPROVED";
        activityDetail['updated_by'] = teacher_id;
        chai.request(NEW_SERVER_URL)
            .post('/activity/activity/edit')
            .send({activity_data: activityDetail})
            .end((err, res) => {
